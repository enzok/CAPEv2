...
def report(request, task_id):
    ...
    try:
        if report.get("info", {}).get("category", "").lower() == "static":
            ...
            report["CAPE"] = list(
                results_db.analysis.aggregate(
                    [{"$match": {"info.id": int(task_id)}}, {"$project": {"_id": 0, "cape_size": {"$size": "$CAPE.sha256"}}}]
                )
            )[0]["cape_size"]
    except:
        report["CAPE"] = 0
    ...
    if category == "sample":
        ...
        if path and category in ("samplezip", "droppedzip", "CAPEZIP", "procdumpzip", "memdumpzip"):
            try:
                ...
                cmd = ["7z", "a", "-y", "-p" + settings.ZIP_PWD, os.path.join(tempfile.gettempdir(), "remove_me", file_name + ".zip"), path]
...
def procdump(request, task_id, process_id, start, end):
    ...
    response = False
    if enabledconf["mongodb"]:
        analysis = results_db.analysis.find_one({"info.id": int(task_id)}, {"procdump": 1, "_id": 0}, sort=[("_id", pymongo.DESCENDING)])
    if es_as_db:
