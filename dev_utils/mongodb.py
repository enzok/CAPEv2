...
@graceful_auto_reconnect
def mongo_find(collection: str, query, projection=False, sort=None, archive=False, limit=None):
    if sort is None:
        sort = [("_id", -1)]
    db = results_db
    if archive:
    ...
    find_by = functools.partial(getattr(db, collection).find, query, sort=sort)
    if projection:
...
@graceful_auto_reconnect
def mongo_find_one(collection: str, query, projection=False, sort=None, archive=False):
    if sort is None:
        sort = [("_id", -1)]
    db = results_db
    if archive:
        db = archive_db
    if projection:
        result = getattr(db, collection).find_one(query, projection, sort=sort)
    else:
        result = getattr(db, collection).find_one(query, sort=sort)
    if result:
...
@graceful_auto_reconnect
def mongo_aggregate(collection: str, query, archive=False):
    db = results_db
    if archive:
        db = archive_db
