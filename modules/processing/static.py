...
try:
    ...
    print(
        "Missed dependey XLMMacroDeobfuscator: pip3 install git+https://github.com/DissectMalware/XLMMacroDeobfuscator.git"
    )
...
class PortableExecutable(object):
    ...
    def _get_icon_info(self):
        ...
        try:
            ...
            rt_icon_idx = [entry.id for entry in self.pe.DIRECTORY_ENTRY_RESOURCE.entries].index(
    ...
    def _get_guest_digital_signers(self):
        ...
        cert_info = os.path.join(
            CUCKOO_ROOT, "storage", "analyses", str(self.results["info"]["id"]), "aux", "DigiSig.json"
        )
                    ...
                    if extension.oid._name == "authorityKeyIdentifier":
                        ...
                        for authority_info in extension.value:
                            if authority_info.access_method._name == "caIssuers":
                                ...
                                cert_data[
                                    "extensions_{}_OCSP".format(extension.oid._name)
                                ] = authority_info.access_location.value
                        ...
                        for index, name in enumerate(extension.value._general_names):
                            if isinstance(name.value, bytes):
                                ...
                                cert_data[
                                    "extensions_{}_{}".format(extension.oid._name, index)
    ...
    def _parse(self, filepath):
        ...
        if vba and vba.detect_vba_macros():
            ...
            if HAVE_VBA2GRAPH and processing_conf.vba2graph.enabled:
                try:
                    vba2graph_path = os.path.join(
                        CUCKOO_ROOT, "storage", "analyses", str(self.results["info"]["id"]), "vba2graph"
                    )
...
class ELF(object):
    ...
    def _get_file_header(self):
        return {
            ...
            "flags": "{}{}".format(
                self._print_addr(self.elf.header["e_flags"]), self._decode_flags(self.elf.header["e_flags"])
            ),
...
class Static(Processing):
    ...
    def run(self):
        ...
        if self.task["category"] == "file":
            ...
            if (
                not HAVE_OLETOOLS
                and "Zip archive data, at least v2.0" in thetype
            ...
            # results for actual zip files.
                ...
                package == "wsf"
                or thetype == "XML document text"
                or self.task["target"].endswith(".wsf")
                or package == "hta"
            ):
