...
def download_file(
    ...
    if not content:
        try:
            ...
            if api:
                ...
                return (
                    "error",
                    render(request, "error.html", {"error": "Provided hash not found on {}".format(service)}),
                )
        ...
        if (
            ...
            if api:
                ...
                return (
                    "error",
                    render(request, "error.html", {"error": "Was impossible to download from {0}".format(service)}),
                )
    ...
    try:
        ...
        if api:
            return (
                "error",
                jsonize({"error": "Error writing {} download file to temporary path".format(service)}, response=True),
            )
        else:
            return (
                "error",
    ...
    if filename:
        ...
        if api:
            ...
            return (
                "error",
                render(request, "error.html", {"error": "File {} not found on {}".format(filename, service)}),
            )
