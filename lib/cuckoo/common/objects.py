...
def is_pe_image(buf):
    ...
    #if ((pNtHeader->FileHeader.Machine == 0) ||
    # (pNtHeader->FileHeader.SizeOfOptionalHeader == 0 || pNtHeader->OptionalHeader.SizeOfHeaders == 0))
    if struct.unpack("<H", nt_headers[4:6]) == 0 or\
            struct.unpack("<H", nt_headers[20:22]) == 0 or struct.unpack("<H", nt_headers[84:86]) == 0:
        return False
    ...
    if struct.unpack("<H", nt_headers[24:26])[0] != OPTIONAL_HEADER_MAGIC_PE and\
            struct.unpack("<H", nt_headers[24:26])[0] != OPTIONAL_HEADER_MAGIC_PE_PLUS:
        return False
