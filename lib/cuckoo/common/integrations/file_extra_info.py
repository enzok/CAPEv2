...
def static_file_info(
    ...
    #    data_dictionary["js"] = EncodedScriptFile(file_path).run()
        ...
        elif selfextract_conf.procyon.deobfuscator_conf and not Path(
                selfextract_conf.procyon.deobfuscator_conf).exists():
...
def generic_file_extractors(
    ...
    for funcname, future in futures.items():
        ...
        try:
            ...
            if old_tool_name:
                log.warning(
                    "Files already extracted from %s by %s. Also extracted with %s", file, old_tool_name, new_tool_name
                )
                continue
